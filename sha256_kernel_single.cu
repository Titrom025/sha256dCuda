...
extern "C" {
    __global__ void sha256d_kernel(beu32* hashes, const beu32* inputs, int num_inputs) {
        int idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx >= num_inputs) return;

        // Load input data for this thread
        beu32 c0 = inputs[idx * 16 + 0];
        beu32 c1 = inputs[idx * 16 + 1];
        beu32 c2 = inputs[idx * 16 + 2];
        beu32 c3 = inputs[idx * 16 + 3];
        beu32 c4 = inputs[idx * 16 + 4];
        beu32 c5 = inputs[idx * 16 + 5];
        beu32 c6 = inputs[idx * 16 + 6];
        beu32 c7 = inputs[idx * 16 + 7];
        beu32 c8 = inputs[idx * 16 + 8];
        beu32 c9 = inputs[idx * 16 + 9];
        beu32 c10 = inputs[idx * 16 + 10];
        beu32 c11 = inputs[idx * 16 + 11];
        beu32 c12 = inputs[idx * 16 + 12];
        beu32 c13 = inputs[idx * 16 + 13];
        beu32 c14 = inputs[idx * 16 + 14];
        beu32 c15 = inputs[idx * 16 + 15];
        ...
        #undef  H
        #define H(i,alpha,magic)  hashes[idx * 8 + i] = hout##i;
        H8(EMPTY,EMPTY);
    }
}
